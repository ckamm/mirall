        ...
        case CSYNC_INSTRUCTION_NEW:
            if (item->_isDirectory) {
                if (item->_direction == SyncFileItem::Down) {
                    ...
                    job->setDeleteExistingFile(item->_type_change);
                    return job;
                } else {
                    auto job = new PropagateRemoteMkdir(this, item);
                    job->setDeleteExisting(item->_type_change);
                    return job;
                }
            }   //fall through
        ...
        case CSYNC_INSTRUCTION_CONFLICT:
            ...
            }
            {
                if (item->_direction != SyncFileItem::Up) {
                    ...
                    job->setDeleteExistingFolder(item->_type_change);
                    return job;
                } else {
...
void OwncloudPropagator::start(const SyncFileItemVector& items)
{
    ...
    foreach(const SyncFileItemPtr &item, items) {
        ...
        if (item->_isDirectory) {
            ...
            if (item->_type_change && item->_direction == SyncFileItem::Up) {
