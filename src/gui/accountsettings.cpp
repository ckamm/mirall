...
void AccountSettings::slotAccountStateChanged(int state)
{
    if (_accountState) {
        ...
        QString server = tr("<a href=\"%1\">%2</a>").arg(account->url().toString(), safeUrl.toString());
        QString serverWithUser = server;
        if (AbstractCredentials *cred = account->credentials()) {
           serverWithUser = tr("<a href=\"%1\">%2</a> as <i>%3</i>").arg(account->url().toString(), safeUrl.toString(), cred->user());
        }
