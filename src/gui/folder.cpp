    ...
    , _saveBackwardsCompatible(false)
{
    ...
    _localDiscoveryDirtyTracker.reset(new LocalDiscoveryDirtyTracker);
    connect(_engine.data(), &SyncEngine::finished,
        _localDiscoveryDirtyTracker.data(), &LocalDiscoveryDirtyTracker::slotSyncFinished);
    connect(_engine.data(), &SyncEngine::itemCompleted,
        _localDiscoveryDirtyTracker.data(), &LocalDiscoveryDirtyTracker::slotItemCompleted);
}
...
void Folder::slotWatchedPathChanged(const QString &path)
{
    ...
    _localDiscoveryDirtyTracker->addTouchedPath(relativePathBytes);
...
void Folder::startSync(const QStringList &pathList)
{
    ...
    if (_folderWatcher && _folderWatcher->isReliable()
        ...
        _engine->setLocalDiscoveryOptions(
            ...
            _localDiscoveryDirtyTracker->localDiscoveryPaths());
        _localDiscoveryDirtyTracker->startSyncPartialDiscovery();
    } else {
        ...
        _localDiscoveryDirtyTracker->startSyncFullDiscovery();
    }
...
LocalDiscoveryDirtyTracker::LocalDiscoveryDirtyTracker()
{

}

void LocalDiscoveryDirtyTracker::addTouchedPath(const QByteArray &relativePath)
{
    qCDebug(lcFolder) << "local discovery: inserted" << relativePath << "due to file watcher";
    _localDiscoveryPaths.insert(relativePath);
}

void LocalDiscoveryDirtyTracker::startSyncFullDiscovery()
{
    _localDiscoveryPaths.clear();
    _previousLocalDiscoveryPaths.clear();
}

void LocalDiscoveryDirtyTracker::startSyncPartialDiscovery()
{
...
const std::set<QByteArray> &LocalDiscoveryDirtyTracker::localDiscoveryPaths() const
{
    return _localDiscoveryPaths;
}

void LocalDiscoveryDirtyTracker::slotItemCompleted(const SyncFileItemPtr &item)
{
...
void LocalDiscoveryDirtyTracker::slotSyncFinished(bool success)
{
    if (success) {
